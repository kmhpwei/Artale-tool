...
if uploaded_file is not None:
    ...
    # 1. 先計算真正有人的團數
    total_real_teams_count = 0
    real_teams_list = []
    
    for team_name, members in final_teams.items():
        if len(members) > 0:
            total_real_teams_count += 1
            real_teams_list.append(get_raw_time(team_name))
            
    # 2. 顯示總數
    st.text(f"預計開 {total_real_teams_count} 團")
    
    # 3. 顯示清單
    for t_name in real_teams_list:
        st.text(f"- {t_name}")
