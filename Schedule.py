...
# ==============================================================================
    ...
    for team_name, members in final_teams.items():
        if len(members) > 0:
            raw_t = get_raw_time(team_name)
            real_open_counts[raw_t] = real_open_counts.get(raw_t, 0) + 1

    # 依照原本的 vote_rank 順序顯示
    for time, count in vote_rank:
        if time in real_open_counts and real_open_counts[time] > 0:
            st.text(f" - {time} (共有 {count} 人) -> 開 {real_open_counts[time]} 團")
